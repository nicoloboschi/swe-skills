...
**Drawing always appends.** A new diagram is placed *below* whatever is already on
the canvas; the whiteboard is a growing board, not a slide that gets overwritten.
Nothing is ever erased unless you ask for it (see [Appending](#appending)).
...
Every `wb.draw()` call adds to the canvas. There is no replace mode: the option
that used to do it (`append: false`) now throws, because silently wiping a board
the user had been building on is not a recoverable mistake.

By default a batch is translated so it starts below the existing content
(`place: "below"`, 120px gap) — draw at whatever coordinates are convenient and it
lands in free space. `place: "right"` puts it beside instead; `place: null` uses
your literal coordinates, which is how you draw *into* an existing diagram.

Removing is explicit and separate:

```js
wb.remove(["api", "db"])   // drop specific elements
wb.clear()                 // empty the whole board
```

The agent only calls those when you ask it to.
...
```js
wb.draw(skeletons, { place = "below", fit = true })  // -> { added, total, offset }
wb.list()                                            // what's on the canvas
wb.bounds()                                          // bounding box, or null if empty
...
`reference.md` is the cheat sheet the agent reads: shapes, arrows, frames, styling
and a palette.
...
- **Appended diagrams landed on top of each other.** Coordinates are absolute and
  there is no auto-layout, so a second drawing at the same `x`/`y` overlaps the
  first. `place: "below"` offsets each batch past the current bounding box.
